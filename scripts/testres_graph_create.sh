...
echo 'Running X...'
python3 ~/klipper/scripts/calibrate_shaper.py /tmp/resonances_x_*.csv -o ~/klipper_config/resonance_testing/shaper_calibrate_x-"`date +"%Y-%m-%d-%H%M%S"`".png

echo 'Running Y...'
python3 ~/klipper/scripts/calibrate_shaper.py /tmp/resonances_y_*.csv -o ~/klipper_config/resonance_testing/shaper_calibrate_y-"`date +"%Y-%m-%d-%H%M%S"`".png
